  ...
  return (
    <div className="form-wrap">
      ...
      <div className="form-actions">
        ...
        <button className="start-project-link" type="button" onClick={() => navigate("/start-project")}>
  ...
  return (
    <section className="email-banner" ref={ref}>
      <div className={`email-top ${vis ? "slide-in" : ""}`}>
        <span className="email-text">HELLO@WEBXTANT.COM</span>
      </div>
      <div className="email-bottom">
        <div className={`connect-label ${vis ? "fade-up delay-2" : ""}`}>Connect</div>
        <div className="social-list">
          {[["01","Instagram","https://www.instagram.com/webxtant"],["02","Behance","https://behance.net"],["03","Github","https://github.com"],["04","LinkedIn","https://linkedin.com"]].map(([num, name, href], i) => (
            <SocialLink key={name} num={num} name={name} href={href} delay={i * 80} visible={vis} />
          ))}
        </div>
      </div>
    </section>
...
function SocialLink({ num, name, href, delay, visible }) {
...
export default function ContactPage() {
  ...
  return (
    <div className="contact-page-wrapper">
      <div className="breadcrumb-bar">HOME / <span>CONTACT</span></div>
      <section className="contact-hero" ref={ref}>
        <div className={`contact-hero-title ${vis ? "fade-up" : ""}`}>
          <h1>Let's make it<br />happen</h1>
        </div>
        <div className={`contact-hero-right ${vis ? "fade-up delay-1" : ""}`}>
